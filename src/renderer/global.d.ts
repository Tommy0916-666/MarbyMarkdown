...
interface Window {
  electronAPI: {
    ...
    // 知识库
    getKbRootPath: () => Promise<string>;
    openKbInExplorer: () => Promise<void>;
    listKnowledgeBases: () => Promise<KnowledgeBaseInfo[]>;
    createKnowledgeBase: (name: string, description?: string) => Promise<KnowledgeBaseInfo | null>;
    deleteKnowledgeBase: (path: string) => Promise<boolean>;
    getKnowledgeBaseConfig: (kbPath: string) => Promise<KnowledgeBaseConfig | null>;
    updateKnowledgeBaseConfig: (kbPath: string, config: Partial<KnowledgeBaseConfig>) => Promise<boolean>;
  };
}
