...
import type { AIOptimizationConfig as AIOptimizationPluginConfig } from "../plugins/ai-optimization";

/** 编辑器配置 */
export interface MarbymarkdownConfig {
  /** 初始 Markdown 内容 */
  content?: string;
  /** 是否只读 */
  readonly?: boolean;
  ...
  /** AI 优化配置 */
  aiOptimizationConfig?: AIOptimizationConfig;
  /** 自定义插件 */
  plugins?: MarbymarkdownPlugin[];
}
...
/** Marbymarkdown 插件 */
export interface MarbymarkdownPlugin {
  name: string;
  init?: (editor: MarbymarkdownEditor) => void;
  destroy?: () => void;
}

/** Marbymarkdown 编辑器实例 */
export interface MarbymarkdownEditor {
  /** ProseMirror 视图 */
  view: EditorView;
  /** 获取 Markdown 内容 */
  getMarkdown: () => string;
