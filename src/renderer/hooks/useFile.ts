...
async function onOpen(result?: { filePath: string; content: string } | null) {
  ...
  try {
    ...
    if (tab && tab.filePath) {
      ...
      // 添加到最近文件记录
      addRecentFile(tab.filePath, tab.name);
      updateTitle();
    }
  } finally {
    nextTick(() => {
...
async function onSave() {
  const { updateTitle } = useTitle();
  const { markdown, filePath, originalContent } = useContent();
  const { updateCurrentTabContent, currentTab, applySavedTabState } = useTab();
  const { triggerManualSave } = useAutoSave();

  // 先更新当前tab的内容
  updateCurrentTabContent(markdown.value);

  // 保存当前tab（会触发 isSaving 动画）
...
async function onSaveAs() {
  ...
  const content = currentTab.value?.content ?? markdown.value;
...
// 创建新文件
function createNewFile(targetDir?: string) {
  const { updateTitle } = useTitle();
  const { markdown, filePath, originalContent } = useContent();
  const { createNewTab } = useTab();

  createNewTab(targetDir);
