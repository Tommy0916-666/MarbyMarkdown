...
/** 插件 Key */
export const placeholderPluginKey = new PluginKey("marbymarkdown-placeholder");
  ...
  return new Plugin({
    ...
    props: {
      attributes(state) {
        ...
        // 构建类名
        let className = "marbymarkdown-editor";
        if (isEmpty(doc)) {
          className += " marbymarkdown-empty";
        }
        if (sourceView) {
