...
export class CodeBlockView implements NodeView {
  ...
  constructor(node: ProseMirrorNode, view: ProseMirrorView, getPos: () => number | undefined) {
    ...
    this.dom.className = "marbymarkdown-code-block";
    ...
    this.editorContainer.className = "marbymarkdown-code-block-editor";
  ...
  private setSourceViewMode(enabled: boolean): void {
    ...
    if (enabled) {
      ...
      if (!this.sourceTextElement) {
        ...
        this.sourceTextElement.className = "marbymarkdown-code-block-source-container";
      ...
      lines.forEach((line) => {
        ...
        lineDiv.className = "marbymarkdown-with-line-number";
  ...
  private handleSourceTextInput(): void {
    ...
    // 从所有子 div 中收集文本内容
    const lines: string[] = [];
    const childDivs = this.sourceTextElement.querySelectorAll("div.marbymarkdown-with-line-number");
      ...
      requestAnimationFrame(() => {
        if (this.sourceTextElement && selection) {
          try {
            ...
            const divs = this.sourceTextElement.querySelectorAll("div.marbymarkdown-with-line-number");
  ...
  private createHeader(language: string): HTMLElement {
    ...
    header.className = "marbymarkdown-code-block-header";

    // 语言选择器
    const langSelector = this.createCustomSelect(supportedLanguages, language, (value) =>
      this.setLanguage(value)
    );
    langSelector.classList.add("marbymarkdown-code-block-lang-select");
    ...
    if (language === "mermaid") {
      ...
      modeSelector.classList.add("marbymarkdown-code-block-mode-select");
      header.appendChild(modeSelector);
    }

    // 复制按钮（hover 时显示）
    const copyBtn = document.createElement("button");
    copyBtn.className = "marbymarkdown-code-block-copy-btn";
    copyBtn.type = "button";
    copyBtn.title = "复制代码块";
  ...
  private createCustomSelect(
    ...
    container.className = "marbymarkdown-custom-select";

    const button = document.createElement("button");
    button.className = "marbymarkdown-custom-select-button";
    button.type = "button";
    const currentOption = options.find((o) => o.value === currentValue);
    button.textContent = currentOption?.label || options[0].label;

    const dropdown = document.createElement("div");
    dropdown.className = "marbymarkdown-custom-select-dropdown";

    for (const option of options) {
      const item = document.createElement("div");
      item.className = "marbymarkdown-custom-select-item";
      ...
      item.addEventListener("click", (e) => {
        ...
        // 更新选中状态
        dropdown.querySelectorAll(".marbymarkdown-custom-select-item").forEach((el) => {
    ...
    button.addEventListener("click", (e) => {
      e.stopPropagation();
      // 关闭所有其他下拉框
      document.querySelectorAll(".marbymarkdown-custom-select.open").forEach((el) => {
        if (el !== container) {
  ...
  private async showContextMenu(e: MouseEvent): Promise<void> {
    ...
    const isHeaderContextMenu = !!(e.target instanceof HTMLElement
      ? e.target.closest(".marbymarkdown-code-block-header")
      : null);
    ...
    separator.className = "marbymarkdown-context-menu-separator";
      ...
      .map((m) => {
        const cls =
          m.index === searchState.currentIndex
            ? "marbymarkdown-search-match marbymarkdown-search-match-current"
            : "marbymarkdown-search-match";
  ...
  private async createMermaidPreview(content: string): Promise<void> {
    if (!this.mermaidPreview) {
      ...
      this.mermaidPreview.className = "marbymarkdown-mermaid-preview";
    ...
    mermaidRenderQueue = mermaidRenderQueue.then(async () => {
      ...
      try {
        ...
        preview.innerHTML = `<div class="marbymarkdown-mermaid-error">Mermaid 渲染错误</div>`;
      }
    });
  ...
  stopEvent(event: Event): boolean {
    ...
    if (event.target instanceof HTMLElement) {
      const isInHeader = event.target.closest(".marbymarkdown-code-block-header");
      if (isInHeader) {
        return false;
      }
    }
