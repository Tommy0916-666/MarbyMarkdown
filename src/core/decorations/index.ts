...
/** 装饰插件 Key */
export const decorationPluginKey = new PluginKey<DecorationPluginState>("marbymarkdown-decorations");

/** CSS 类名映射 */
export const SYNTAX_CLASSES: Record<string, string> = {
  strong: "marbymarkdown-strong",
  emphasis: "marbymarkdown-emphasis",
  code_inline: "marbymarkdown-code-inline",
  strikethrough: "marbymarkdown-strikethrough",
  link: "marbymarkdown-link",
  highlight: "marbymarkdown-highlight",
  math_inline: "marbymarkdown-math-inline",
  heading: "marbymarkdown-heading", // 标题
  strong_emphasis: "marbymarkdown-strong-emphasis", // 粗斜体
  escape: "marbymarkdown-escape", // 转义
  sub: "marbymarkdown-sub", // 下标
  sup: "marbymarkdown-sup", // 上标
  html_inline: "marbymarkdown-html-inline", // 行内 HTML
};
...
export function computeDecorations(
  ...
  // 源码模式下跳过所有装饰计算：
  // - 语法标记通过 .marbymarkdown-syntax-marker 类（mark 自带）已有正确样式
  ...
  for (const region of syntaxRegions) {
    ...
    if (!shouldShow) {
        ...
        if (hashEnd > 0 && hashEnd < text.length) {
          decorations.push(
            Decoration.inline(region.from, region.from + hashEnd, {
              class: "marbymarkdown-syntax-hidden",
      ...
      decorations.push(
        Decoration.inline(region.from, region.to, {
          class: "marbymarkdown-syntax-visible",
        })
      );
    }
  }
    ...
    if (!cursorInMath && !sourceView && mathRegion.content.trim()) {
      ...
      decorations.push(
        Decoration.inline(mathRegion.from, mathRegion.to, {
          class: "marbymarkdown-math-source-hidden",
        })
      );
      ...
      if (renderedHtml) {
        ...
        widget.className = "marbymarkdown-math-rendered";
