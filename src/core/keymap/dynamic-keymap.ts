...
import { triggerAICompletion, acceptAICompletion } from "../plugins/ai-completion";
  ...
  function getHandler() {
    ...
    if (configStr !== lastConfigStr || !cachedHandler) {
      ...
      for (const shortcut of DEFAULT_SHORTCUTS) {
        ...
        // AI 续写：特殊处理，直接调用 triggerAICompletion
        if (shortcut.id === "aiContinue") {
          bindings[boundKey] = (state: any, dispatch: any, view: any) => {
            if (view) {
              const result = triggerAICompletion(view);
              return result;
            }
            return false;
          };
        } else if (shortcut.id === "aiAccept") {
          bindings[boundKey] = (state: any, dispatch: any, view: any) => {
            if (view) {
              const result = acceptAICompletion(view);
              return result;
            }
            return false;
          };
