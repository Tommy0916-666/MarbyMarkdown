...
import { getMacaronFlowController } from "@/renderer/utils/macaronFlowController";
...
function setTheme(theme: ThemeName = currentTheme.value) {
  ...
  // 获取马卡龙流光控制器
  const macaronController = getMacaronFlowController();

  // 先停止之前可能激活的马卡龙流光
  macaronController.stop();
  ...
  html.style.removeProperty("--background-color-1");
  html.style.removeProperty("--background-color-2");
  html.style.removeProperty("--background-color-3");
  html.style.removeProperty("--secondary-color");
  html.style.removeProperty("--text-color");
  html.style.removeProperty("--text-color-1");
  html.style.removeProperty("--text-color-2");
  html.style.removeProperty("--text-color-3");
  html.style.removeProperty("--crepe-color-background");
  html.style.removeProperty("--crepe-color-on-background");
  html.style.removeProperty("--crepe-color-surface");
  html.style.removeProperty("--crepe-color-surface-low");
  html.style.removeProperty("--crepe-color-primary");
  html.style.removeProperty("--crepe-color-secondary");
  html.style.removeProperty("--crepe-color-inline-code");
  ...
  // 如果是马卡龙流光主题，启动动态色彩
  if (theme === "macaron-flow") {
    macaronController.start();
  }
