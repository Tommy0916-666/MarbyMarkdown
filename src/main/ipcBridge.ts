...
export function registerIpcOnHandlers() {
  ipcMain.on("set-title", (event, filePath: string | null) => {
    ...
    if (!targetWin) return;
    const title = filePath ? `marbymarkdown - ${path.basename(filePath)}` : "marbymarkdown - Untitled";
  ...
  ipcMain.handle(
    ...
    async (
      event,
      {
        ...
        imageLocalPath,
        defaultDir,
      }: { content: string; fileTraits?: FileTraits; imageLocalPath?: string; defaultDir?: string }
    ) => {
      const parentWin = BrowserWindow.fromWebContents(event.sender);
      if (!parentWin) return null;
      ...
      if (defaultDir) {
        saveOptions.defaultPath = defaultDir;
      }
      const { canceled, filePath } = await dialog.showSaveDialog(parentWin, saveOptions);
    ...
    async (
      ...
      const preventCutOffStyle = `
        <style>
          ...
          .marbymarkdown-code-block .cm-editor span{
...
export function registerGlobalIpcHandlers() {
  ...
  // 保存临时回滚文件
  ipcMain.handle("rollback:saveTemp", async (_event, filePath: string, content: string) => {
    try {
      const tempDir = path.join(app.getPath("userData"), "rollback");
      if (!fs.existsSync(tempDir)) {
        fs.mkdirSync(tempDir, { recursive: true });
      }
      // 用 base64 编码原文件路径作为文件名
      const tempFileName = Buffer.from(filePath).toString("base64") + ".md";
      const tempFilePath = path.join(tempDir, tempFileName);
      fs.writeFileSync(tempFilePath, content, "utf-8");
      return true;
    } catch (error) {
      console.error("保存临时回滚文件失败:", error);
      return false;
    }
  });

  // 读取临时回滚文件
  ipcMain.handle("rollback:loadTemp", async (_event, filePath: string) => {
    try {
      const tempDir = path.join(app.getPath("userData"), "rollback");
      const tempFileName = Buffer.from(filePath).toString("base64") + ".md";
      const tempFilePath = path.join(tempDir, tempFileName);
      ...
      console.error("读取临时回滚文件失败:", error);
      return null;
    }
  });

  // 删除临时回滚文件
  ipcMain.handle("rollback:deleteTemp", async (_event, filePath: string) => {
    try {
      const tempDir = path.join(app.getPath("userData"), "rollback");
      const tempFileName = Buffer.from(filePath).toString("base64") + ".md";
      const tempFilePath = path.join(tempDir, tempFileName);
      ...
      console.error("删除临时回滚文件失败:", error);
      return false;
    }
  });
  ...
  // 知识库：获取根目录路径
  ipcMain.handle("kb:getRootPath", async () => {
    const { getKnowledgeBaseRoot } = await import("./kbManager");
    return getKnowledgeBaseRoot();
  });

  // 知识库：在资源管理器中打开
  ipcMain.handle("kb:openInExplorer", async () => {
    const { openKnowledgeBaseInExplorer } = await import("./kbManager");
    openKnowledgeBaseInExplorer();
  });

  // 通用：在资源管理器中打开指定路径
  ipcMain.handle("kb:openPath", async (_event, targetPath: string) => {
    shell.openPath(targetPath);
  });

  // 知识库：列出所有知识库
  ipcMain.handle("kb:list", async () => {
    const { listKnowledgeBases } = await import("./kbManager");
    return listKnowledgeBases();
  });

  // 知识库：创建知识库
  ipcMain.handle("kb:create", async (_event, params: { name: string; description?: string }) => {
    const { createKnowledgeBase } = await import("./kbManager");
    return createKnowledgeBase(params);
  });
  
  // 知识库：获取知识库配置
  ipcMain.handle("kb:getConfig", async (_event, kbPath: string) => {
    const { getKnowledgeBaseConfig } = await import("./kbManager");
    return getKnowledgeBaseConfig(kbPath);
  });
  
  // 知识库：更新知识库配置
  ipcMain.handle("kb:updateConfig", async (_event, kbPath: string, config: any) => {
    const { updateKnowledgeBaseConfig } = await import("./kbManager");
    return updateKnowledgeBaseConfig(kbPath, config);
  });

  // 知识库：删除知识库
  ipcMain.handle("kb:delete", async (_event, { path: kbPath }: { path: string }) => {
    const { deleteKnowledgeBase } = await import("./kbManager");
    return deleteKnowledgeBase(kbPath);
  });
}
