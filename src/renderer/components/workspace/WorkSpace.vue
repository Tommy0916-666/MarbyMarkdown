...
const { recentFiles, recentFilesConfig, removeRecentFile, addRecentFile } = useRecentFiles();
const { startAutoSave } = useAutoSave();

// 最近文件列表是否折叠
const recentFilesCollapsed = ref(false);
...
onMounted(() => {
  ...
  emitter.on("file:opened", (filePath: string, fileName: string) => {
    addRecentFile(filePath, fileName);
  });
});
onUnmounted(() => {
  document.removeEventListener("click", onDocumentClick);
  emitter.off("file:opened");
});
...
<template>
  <div class="WorkSpace" @contextmenu.prevent="onBlankContextMenu">
    ...
    <!-- 最近打开的文件列表 -->
    <div v-if="recentFilesConfig.enabled && recentFiles.length > 0" class="recent-files-section">
      <div class="section-header" @click="recentFilesCollapsed = !recentFilesCollapsed">
        <AppIcon name="folder-opened" class="section-icon" />
        <span class="section-title">最近打开</span>
        <AppIcon
          name="arrow-right"
          class="collapse-icon"
          :class="{ collapsed: recentFilesCollapsed }"
        />
      </div>
      <div v-if="!recentFilesCollapsed" class="recent-files-list">
        <div
          v-for="file in recentFiles"
          :key="file.path"
          class="recent-file-item"
          :class="{ active: currentTab?.filePath === file.path }"
          @click="openFile(file.path)"
        >
          <AppIcon name="markdown" class="file-icon" />
          <span class="file-name">{{ file.name }}</span>
          <button
            class="remove-btn"
            @click.stop="removeRecentFile(file.path)"
          >
            <AppIcon name="close" />
          </button>
        </div>
