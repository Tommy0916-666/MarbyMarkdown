...
/** 插件 Key */
export const pastePluginKey = new PluginKey("marbymarkdown-paste");
...
function getStoredImageConfig(): StoredImageConfig {
  try {
    const rawConfig = localStorage.getItem("marbymarkdown-config");
    if (!rawConfig) return {};
...
export function createPastePlugin(config: PastePluginConfig = {}): Plugin {
  const parser = new MarkdownParser(marbymarkdownSchema);
