...
import "@/core/styles/marbymarkdown.css";
...
const {
  continueConfig: aiContinueConfig,
  continueEffectiveConfig: aiConfig,
  isContinueEnabled: aiEnabled,
  optimizeConfig,
...
let editor: MarbymarkdownEditor | null = null;
...
function createEditorInstance() {
  ...
  const config: MarbymarkdownConfig = {
    ...
    aiConfig: {
      ...
      get mode() {
        return aiContinueConfig.value.mode;
      },
      get debounceWait() {
        return aiContinueConfig.value.debounceWait;
      },
    ...
    aiOptimizationConfig: {
      get enabled() {
        return isOptimizeEnabled.value;
      },
      get stylePreset() {
        return optimizeConfig.value.stylePreset;
      },
      get customPrompt() {
        return optimizeConfig.value.customPrompt;
      },
      optimize: async (text: string, style: any, customPrompt: string) => {
        return await AIService.optimize(aiConfig.value, text, style, customPrompt);
      },
    },
  };
...
<template>
  <div
    class="editor-box marbymarkdown-editor-instance"
    :data-tab-id="tab.id"
    :data-active="isActive ? 'true' : 'false'"
  >
    <div ref="scrollViewRef" class="scrollView marbymarkdown" @scroll="updateScrollRatio">
...
<style>
/* Marbymarkdown 编辑器全局样式覆盖 */
.marbymarkdown-editor {
  background: var(--background-color-1);
...
.marbymarkdown-editor h1,
.marbymarkdown-editor h2,
.marbymarkdown-editor h3,
.marbymarkdown-editor h4,
.marbymarkdown-editor h5,
.marbymarkdown-editor h6 {
  color: var(--text-color-1);
}

.marbymarkdown-editor blockquote {
  border-left-color: var(--border-color-1);
  background: var(--background-color-2);
}

.marbymarkdown-code-block {
  background: var(--background-color-2);
  border-color: var(--border-color-1);
}

.marbymarkdown-code-block-header {
  background: var(--background-color-3);
}

.marbymarkdown-editor table th,
.marbymarkdown-editor table td {
  border-color: var(--border-color-1);
}
...
.marbymarkdown-editor hr {
  background: var(--border-color-1);
}

.marbymarkdown-syntax-marker {
