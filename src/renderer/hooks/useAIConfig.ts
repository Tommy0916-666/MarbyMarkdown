...
// 知识库 AI 配置
export interface AIKnowledgeBaseConfig extends AIFunctionConfig {}
...
export interface AIFullConfig {
  ...
  knowledgeBase: AIKnowledgeBaseConfig;
}
...
export function useAIConfig() {
  ...
  // 知识库配置
  const knowledgeBaseConfig = computed(() => fullConfig.value.knowledgeBase);
  const knowledgeBaseEffectiveConfig = computed(() => getEffectiveFunctionConfig(fullConfig.value.knowledgeBase, fullConfig.value.global));
  const isKnowledgeBaseEnabled = computed(() => fullConfig.value.knowledgeBase.enabled);
  ...
  // 更新知识库配置
  const updateKnowledgeBaseConfig = (updates: Partial<AIKnowledgeBaseConfig>) => {
    fullConfig.value.knowledgeBase = { ...fullConfig.value.knowledgeBase, ...updates };
  };
  ...
  const resetKnowledgeBaseToDefault = () => {
    fullConfig.value.knowledgeBase = { ...defaultAIFullConfig.knowledgeBase };
  };
