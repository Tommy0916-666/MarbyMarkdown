...
import { marbymarkdownSchema } from "./schema";
...
import { createAICompletionPlugin, triggerAICompletion } from "./plugins/ai-completion";
import { createAIOptimizationPlugin, triggerAIOptimization } from "./plugins/ai-optimization";
...
import type { MarbymarkdownConfig, MarbymarkdownEditor as IMilkupEditor, MarbymarkdownPlugin } from "./types";
import {
  insertTable,
  addRowBefore,
  addRowAfter,
...
/** 编辑器默认配置 */
const defaultConfig: MarbymarkdownConfig = {
  content: "",
  readonly: false,
  sourceView: false,
};
 ...
 * Marbymarkdown 编辑器类
 */
export class MarbymarkdownEditor implements IMilkupEditor {
  view: EditorView;
  private config: MarbymarkdownConfig;
  private schema: Schema;
  private parser: MarkdownParser;
  private serializer: MarkdownSerializer;
  private plugins: MarbymarkdownPlugin[] = [];
  ...
  constructor(container: HTMLElement, config: MarbymarkdownConfig = {}) {
    this.config = { ...defaultConfig, ...config };
    this.schema = marbymarkdownSchema;
  ...
  private createPlugins(): Plugin[] {
    ...
    // AI 续写插件（如果配置了）
    console.log("🔍 Checking AI Config:", !!this.config.aiConfig, "AI Optimization Config:", !!this.config.aiOptimizationConfig);
    if (this.config.aiConfig) {
      const aiConfig = this.config.aiConfig;
      console.log("✅ Adding AI Completion plugin");
      plugins.push(createAICompletionPlugin(() => aiConfig));
    }
    // AI 优化插件（如果配置了）
    if (this.config.aiOptimizationConfig) {
      const aiOptConfig = this.config.aiOptimizationConfig;
      console.log("✅ Adding AI Optimization plugin");
      plugins.push(createAIOptimizationPlugin(() => aiOptConfig));
    }
   ...
   * 更新配置
   */
  updateConfig(config: Partial<MarbymarkdownConfig>): void {
  ...
  private static readonly MOD_KEY = MarbymarkdownEditor.IS_MAC ? "⌘" : "Ctrl";
  ...
  private getLinkHref(linkEl: HTMLAnchorElement): string {
    ...
    if (sibling && sibling.matches('span.marbymarkdown-syntax[data-syntax-type="link"]')) {
  ...
  private initLinkHandler(): void {
    ...
    dom.addEventListener(
      ...
      (e: Event) => {
        ...
        const modPressed = MarbymarkdownEditor.IS_MAC ? me.metaKey : me.ctrlKey;
  ...
  private handleContextMenu(view: EditorView, event: MouseEvent): boolean {
    ...
    if (
      target.closest(".marbymarkdown-code-block-editor") ||
      target.closest(".marbymarkdown-code-block-header")
    ) {
      return false; // 让代码块处理
    }
  ...
  private createContextMenuSeparator(): HTMLElement {
    ...
    sep.className = "marbymarkdown-context-menu-separator";
    return sep;
  }
  ...
  private createContextMenuItemWithSubmenu(
    ...
    item.className = "marbymarkdown-context-menu-item has-submenu";
    item.textContent = label;

    const submenu = document.createElement("div");
    submenu.className = "marbymarkdown-context-menu-submenu";
  ...
  private buildTableGridPicker(container: HTMLElement): void {
    ...
    picker.className = "marbymarkdown-table-grid-picker";
  ...
  private async showContextMenu(e: MouseEvent): Promise<void> {
    ...
    menu.className = "marbymarkdown-context-menu";
    ...
    // AI 优化（如果有选区且配置了）
    console.log("🔍 Checking AI Optimization menu: hasSelection=", hasSelection, "config=", !!this.config.aiOptimizationConfig);
    if (hasSelection && this.config.aiOptimizationConfig) {
      menu.appendChild(this.createContextMenuSeparator());
      const optimizeItem = this.createContextMenuItem("AI 优化", false, () => {
        console.log("🖱️ AI Optimization menu clicked");
        this.hideContextMenu();
        this.view.focus();
        if (this.config.aiOptimizationConfig) {
          console.log("🚀 Calling triggerAIOptimization");
          triggerAIOptimization(this.view, () => this.config.aiOptimizationConfig!);
        }
      });
      menu.appendChild(optimizeItem);
    }
  ...
  private createContextMenuItem(
    ...
    const item = document.createElement("div");
    item.className = "marbymarkdown-context-menu-item";
    if (disabled) {
  ...
  private createContextMenuItemWithShortcut(
    ...
    if (shortcutActionId) {
      ...
      if (shortcut) {
        ...
        shortcutSpan.className = "marbymarkdown-context-menu-shortcut";
  ...
  private createSearchPanel(container: HTMLElement): void {
    ...
    wrapper.className = "marbymarkdown-search-wrapper";

    const panel = document.createElement("div");
    panel.className = "marbymarkdown-search-panel";

    // ---- 搜索行 ----
    const searchRow = document.createElement("div");
    searchRow.className = "marbymarkdown-search-row";
    ...
    replaceRow.className = "marbymarkdown-replace-row hidden";
  ...
  private hideContextMenu(): void {
    ...
    // 移除其他可能存在的右键菜单
    document.querySelectorAll(".marbymarkdown-context-menu").forEach((el) => el.remove());
  }
  ...
  private getCustomKeyMap(): ShortcutKeyMap {
    try {
      const raw = localStorage.getItem("marbymarkdown-config");
      if (raw) {
...
/**
 * 创建 Marbymarkdown 编辑器
 */
export function createMilkupEditor(container: HTMLElement, config?: MarbymarkdownConfig): MarbymarkdownEditor {
  return new MarbymarkdownEditor(container, config);
}
