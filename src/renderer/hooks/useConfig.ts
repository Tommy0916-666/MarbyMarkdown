...
interface AppConfig extends Record<string, any> {
  ...
  workspace: {
    ...
    restoreLastFiles: boolean;
    lastOpenedFiles: Array<{ filePath: string; scrollRatio: number }>;
  };
  recentFiles: {
    enabled: boolean;
    files: Array<{ path: string; name: string; lastOpened: number }>;
    maxFiles: number;
  };
  autoSave: {
    enabled: boolean;
  ...
  knowledgeBase: {
    defaultMode: "editor" | "knowledgeBase";
...
const config = useStorage<AppConfig>("marbymarkdown-config", defaultConfig, localStorage, {
  serializer: {
    read: (value: string) => {
      try {
