...
// 保存指定tab
async function saveTab(tab: Tab, isAutoSave: boolean = false): Promise<boolean> {
  ...
  try {
    ...
    if (saved) {
      // 如果是自动保存，只更新 isModified，不重载编辑器，避免打断用户输入
      if (isAutoSave) {
        tab.isModified = false;
...
// 保存当前tab
async function saveCurrentTab(isAutoSave: boolean = false): Promise<boolean> {
  const currentTab = getCurrentTab();
  return saveTab(currentTab!, isAutoSave);
}
...
async function openFile(filePath: string): Promise<Tab | null> {
  try {
    ...
    if (existingTab) {
      ...
      // 发送事件添加到最近文件记录
      emitter.emit("file:opened", existingTab.filePath!, existingTab.name);
      return existingTab;
    }
    ...
    // 发送事件添加到最近文件记录
    if (openedTab && openedTab.filePath) {
      emitter.emit("file:opened", openedTab.filePath, openedTab.name);
    }
...
// 创建新文件tab
function createNewTab(targetDir?: string): Tab {
  const tab: Tab = {
    id: randomUUID(),
    name: defaultName,
    filePath: null,
...
function closeAllTabs() {
  ...
  scheduleNewlyLoadedCleanup(newTab.id);
  setCurrentMarkdownFilePath(null);
}

// 清空所有tab（不创建新文档，用于知识库模式）
...
function closeWithConfirm(id: string) {
  ...
  // 如果没有未保存内容
  if (isLastTab) {
    // 如果是最后一个tab，先检查是否在编辑器模式
    // 只有在编辑器模式下才关闭应用
    emitter.emit("tab:check-editor-mode", {
      tabId: id,
      isLastTab,
    });
  } else {
...
// 保存当前打开的文件列表
function saveOpenedFiles() {
  const { config, setConf } = useConfig();
  const openedFiles = tabs.value
  ...
  setConf("workspace", { ...config.value.workspace, lastOpenedFiles: openedFiles });
}

// 恢复上次打开的文件
async function restoreLastOpenedFiles() {
  const { config } = useConfig();
  
  // 如果没有启用恢复功能或者没有保存的文件，直接返回
  if (!config.value.workspace.restoreLastFiles || config.value.workspace.lastOpenedFiles.length === 0) {
    return false;
  }

  // 我们不要去删除默认 tab！让 openFile 自己处理复用逻辑
  let firstRestoredTabId: string | null = null;

  for (const fileInfo of config.value.workspace.lastOpenedFiles) {
    try {
      const tab = await openFile(fileInfo.filePath);
      if (tab) {
        tab.scrollRatio = fileInfo.scrollRatio;
        if (!firstRestoredTabId) {
          firstRestoredTabId = tab.id;
        }
      }
    } catch (error) {
      console.error(`无法恢复文件: ${fileInfo.filePath}`, error);
    }
  }

  return firstRestoredTabId !== null;
}
