...
const saveStatus = computed(() => {
  ...
  // 检查是否有未保存的修改
  if (currentTab.value?.isModified) {
...
async function updateRollbackStatus() {
  ...
  if (!currentTab.value?.filePath || !enabled || !enableRollback) {
    hasRollback.value = false;
    return;
  }
  
  const rollbackExists = await hasRollbackContent(currentTab.value.filePath);
...
async function handleRollback() {
  if (!currentTab.value?.filePath) return;
  const rollbackContent = await getRollbackContent(currentTab.value.filePath);
  if (!rollbackContent) return;

  // 回滚内容，标记为已修改，让用户决定是否保存
  currentTab.value.content = rollbackContent;
  currentTab.value.isModified = true;
  // 发送事件，让编辑器更新
...
<template>
  <div class="StatusBarBox">
    ...
    <div class="right-section">
      ...
      <button
        v-if="(autoSaveConfig?.enabled ?? true) && (autoSaveConfig?.enableRollback ?? true) && currentTab?.filePath && hasRollback"
        class="rollback-btn"
